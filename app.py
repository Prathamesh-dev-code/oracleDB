...
def translate_nl_to_sql(nl_query,chat_log):
    ...
    # Prepare conversational chat log for context-aware responses
    conversation_log_prompt = "Previous conversation history:\n"
    for message in chat_log:
        if 'user' in message:
            conversation_log_prompt += f"User: {message['user']}\n"
        if 'system' in message:
            if 'sql_query' in message['system']:
                conversation_log_prompt += f"System (SQL Query): {message['system']['sql_query']}\n"
            if 'summary' in message['system']:
                conversation_log_prompt += f"System (Summary): {message['system']['summary']}\n"
            if 'error' in message['system']:
                conversation_log_prompt += f"System (Error): {message['system']['error']}\n"
    ...
    ### Detect Partial Keywords and Autocomplete:
    - If the user provides incomplete or partial terms, identify the closest match in the database by searching relevant columns and suggest the correct word or phrase.
    - Use a `LIKE` query in Oracle SQL to find potential matches based on user input. Detect close matches and replace the partial keyword with the correct one before generating the SQL query.
    ...
    full_prompt = f"{conversation_log_prompt}\n{schema_prompt}\n\n{semantic_prompt}\n\nNatural Language Query: \"{nl_query}\"\n\nSQL Query:"
...
def generate_summary(df, user_query):
    ...
    summary_prompt = f"""
    You are a data expert. Please generate a brief summary in 5 lines based on this result and the original query.
...
def chat():
    ...
    if request.method == 'POST':
        ...
        sql_query, explanation = translate_nl_to_sql(user_query,chat_log)
