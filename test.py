...
def translate_nl_to_sql(nl_query):
    ...
    for log in chat_log:
        if 'user' in log:
            chat_context += f"User: {log['user']}\n"
        elif 'system' in log:
            chat_context += f"Assistant: {log['system'].get('summary', log['system'].get('error', ''))}\n"
    ...
    if request.method == 'POST':
        ...
        if sql_query.strip():
            ...
            if error:
                ...
                summary = generate_summary(df, user_query)  # Pass both df and user_query
            ...
            chat_log.append({'system': {'error': "No valid SQL query found."}})
...
def transcribe_audio():
    ...
    with tempfile.NamedTemporaryFile(delete=True) as temp_audio_file:
        ...
        # Use OpenAI Whisper model to transcribe
